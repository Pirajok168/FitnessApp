package ru.fitness.mobile.presentation.navigation
...
abstract class Route<T>(route: String? = null, private val navArgs: Map<T, NavType<*>> = emptyMap()) {
    companion object {
        ...
        const val DEEP_LINK_HOST = "fitness.ru"
    }
    ...
    fun navigate(args: (key: T) -> Any? = { null }): String =
        provideRoute + navArgs.mapNotNull { args(it.key)?.let { arg -> "${it.key}=$arg" } }.let { it.joinToString(separator = "&", prefix = if (it.any()) "?" else "") }
